...
class Galerias extends MY_Controller{
    ...
    function syncGaleriasDiarias(){
        ...
        $carpetas=$this->carpetasDirectorio("galeriasdiarias");

        $noDir=count($carpetas);
        for ($i=0; $i<$noDir; $i++){

           if($carpetas[$i]!=".DS_Store" && $carpetas[$i]!=".." && $carpetas[$i]!="."){
        
            $imagenes=$this->imagenesDirectorio("galeriasdiarias/".$carpetas[$i]);
            $this->data_model($imagenes, $carpetas[$i]);
           } 
       }
    ...
    public function imagenesDirectorio($foldername){
        $directorios = SITE_HARD_ROOT_FILE."imagenes/".$foldername;
        $imagenesCarpetaDiarios = scandir($directorios, 1);
        return $imagenesCarpetaDiarios;
    }

     public function carpetasDirectorio($dir){
        $directorios = SITE_HARD_ROOT_FILE."imagenes/".$dir;
        $directoriosCarpeta = scandir($directorios, 1);
        return $directoriosCarpeta;
    }
    ...
    public function data_model($imagenes, $foldername)
    {
        ...
        $destino= strtolower( 'imagenes/'.$foldername).'/';
        /*$this->load->helper('date');
        $datestring = "%Y-%m-%d";
        $time = time();
        $fechaActual=mdate($datestring, $time);*/
